...
function ArithmeticBM(targetString:string,findString:string):number{
    ...
    let findLen=findString.length;

    let left=0,k=findLen-1,right=k; //left、right表示target的索引 k表示find的索引
    let m=0,n=0;
    ...
    while(k>=0 && right<targetLen){
        ...
        if(targetString[right]==findString[k]){
            ...
            if(k<findLen-1){
                let goodSuffix=findString.substring(k+1);
                let tempStr=findString.substring(0,k+1);

                var start=ArithmeticBM(goodSuffix,tempStr); //start>0说明有匹配字符
                if(start>=0) m=k+1-start; //后移位数
                else if(goodSuffix[goodSuffix.length-1]==tempStr[0]){
                    m=findString.length-1;
                }
                else m=findString.length;
            }
            //坏字符规则 坏字符位置-上一次出现位置
            n=1;
            while(k-n>=0){
...
testFunctionTime(ArithmeticBM,str1,str2);
testFunctionTime(()=>{return str1.indexOf(str2)});